...
const AdminPage = () => {
    ...
    const fetchRecipes = async () => {
        try {
            const response = await axios.get(`http://localhost:5000/api/recipes?lang=${language}`);
    ...
    const fetchCountries = async () => {
        try {
            const response = await axios.get(`http://localhost:5000/api/countries?lang=${language}`);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            if (editingRecipe) {
                await axios.put(`http://localhost:5000/api/recipes/${editingRecipe.recipe_id}`, data, {
    ...
    const handleEdit = (recipe) => {
        ...
        setFormData({
            ...
            imageFile: null,
            imagePreview: recipe.image_url ? `http://localhost:5000${recipe.image_url}` : null,
    ...
    const handleCreateCountry = async (e) => {
        e.preventDefault();
        try {
            await axios.post('http://localhost:5000/api/countries', newCountry);
